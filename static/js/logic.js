...
// Create Map, Passing In grayscaleMap & earthquakes as Default Layers to Display on Load
var myMap = L.map("map", {
    center: [37.09, -95.71],
    zoom: 4,
...
d3.json(earthquakesURL, function(earthquakeData) {
    ...
    legend.onAdd = function() {
        ...
        div.innerHTML += '<h3 class="square"> Magnitude</h3>'

        for (var i = 0; i < magnitudeLevels.length; i++) {
            div.innerHTML +=
            '<i class="square" style="background: ' + chooseColor(magnitudeLevels[i] + 1) + '"></i> &nbsp' +
            '<span class="square">' + magnitudeLevels[i] + (magnitudeLevels[i + 1] ? '&ndash;' + magnitudeLevels[i + 1] + '</span>' + '<br>' : '+');
        }
        return div;
    };
